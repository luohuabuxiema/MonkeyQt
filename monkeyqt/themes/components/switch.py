# -*- coding: utf-8 -*-
"""
ThemedSwitch — 支持 67 种 UI 风格的开关组件
"""
...
from ..style_utils import draw_liquid_glass, qcolor


class ThemedSwitch(QWidget):
    """
    风格化开关组件。

    用法:
        switch = ThemedSwitch(checked=True)
        switch.toggled.connect(lambda v: print("Toggled:", v))
    """
    toggled = Signal(bool)

    def __init__(self, checked=False, width=44, height=24, parent=None):
    ...
    def paintEvent(self, event):
        ...
        inactive_color = QColor("#DCDFE6") if not t.is_dark() else QColor("#4C4D4F")
